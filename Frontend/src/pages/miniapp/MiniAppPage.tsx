...
export function MiniAppPage() {
  ...
  const [playlistDieuChinh, setPlaylistDieuChinh] = useState<IptvChannelList | null>(null);
  ...
  async function xoaPlaylist(l: IptvChannelList) {
    ...
    try {
      ...
      if (playlistDieuChinh?.id === l.id) setPlaylistDieuChinh(null);
  ...
  return (
    <AppShell activeTab="miniapp">
      <div className="ma">
        ...
        <div className={`ma-col${trangMobile === 1 ? " ma-mobile-active" : ""}`}>
          ...
          <div className="ma-col-actions">
            ...
            <button
              className="ma-pill ma-pill-ghost"
              type="button"
              disabled={chon === null || !chon.canEdit}
              title={chon !== null && !chon.canEdit ? "Bạn chỉ có quyền xem playlist này" : undefined}
              onClick={() => chon && setPlaylistDieuChinh(chon)}
            >
              Điều chỉnh
            </button>
      ...
      {playlistDieuChinh && (
        <EditPlaylistDialog
          playlist={playlistDieuChinh}
          laAdmin={laAdmin}
          onClose={() => setPlaylistDieuChinh(null)}
          onUpdated={(daCapNhat) => {
            setPlaylistDieuChinh(null);
            setLists((truoc) => (truoc ?? []).map((l) => (l.id === daCapNhat.id ? daCapNhat : l)));
            setGhiChu("Đã cập nhật playlist.");
            setError(null);
            // Link moi co the da them/xoa/doi URL cua kenh. Nap lai cay ben
            // phai thay vi giu du lieu cu trong bo nho.
            void iptvApi
              .listGroups(daCapNhat.id)
              .then((res) => setGroups(res.data))
              .catch((err) =>
                setError(
                  extractApiError(
                    err,
                    "Playlist đã cập nhật nhưng chưa tải lại được danh sách kênh",
                  ),
                ),
              );
          }}
